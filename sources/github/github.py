...
def handler(event, context):
    ...
    r = requests.get('https://api.github.com/meta', headers=headers)
    ...
    if r.status_code == 200:
        ...
        for key, value in data.items():
            if key != 'verifiable_password_authentication' and key != 'ssh_key_fingerprints' and key != 'ssh_keys' and key != 'domains':
                for v in value:
                    hostmask = v.split('/')
                    ...
                    f.write(os.environ['SOURCE']+','+key+','+os.environ['SOURCE']+','+v+','+str(firstip)+','+str(lastip)+'\n')
