...
def handler(event, context):
    ...
    r = requests.get('https://ip-ranges.salesforce.com/ip-ranges.json', headers=headers)
    ...
    if r.status_code == 200:
        ...
        for cidr in output['prefixes']:
            for ip in cidr['ip_prefix']:
                netrange = ipaddress.IPv4Network(ip)
                first, last = netrange[0], netrange[-1]
                firstip = int(ipaddress.IPv4Address(first))
                lastip = int(ipaddress.IPv4Address(last))
                f.write(os.environ['SOURCE']+','+now+','+ip+','+str(firstip)+','+str(lastip)+','+cidr['region']+',-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-,-\n')
